...
def unsharp_mask(img: np.ndarray, sigma: float, strength: float) -> np.ndarray:
    ...
    # img + strength*(img - blurred) = img*(1+strength) + blurred*(-strength)
    ...
    try:
        ...
        with mp_face_mesh.FaceMesh(
            ...
            if results.multi_face_landmarks:
                ...
                # Left eye: 33,133,160,159,158,157,173,246,161,163,144,145,153,154,155
                # Right eye: 362,263,387,386,385,384,398,466,388,390,373,374,380,381,382
                # Nose: 1,2,3,4,5,6,168,195,197,236,456,248,281,278,279,280,457,274,275,19,94
                ...
                # Get bounding points for the zone
                points = []
                ...
                # Create convex hull mask for eye+nose region
                hull = cv2.convexHull(points)
                cv2.fillConvexPoly(mask, hull, 1.0)

                # Expand mask slightly with dilation
...
def get_face_encoding(img: np.ndarray):
    ...
    rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    locations = fr.face_locations(rgb, number_of_times_to_upsample=2)
    if not locations:
        return None
...
if __name__ == "__main__":
    ...
    for fp in face_paths:
        ...
        # ---- BONUS: skip enhancement if already sharp enough ----
        raw_sharpness = sharpness(raw)
        if raw_sharpness > SHARP_SKIP_THRESHOLD:
            # Already sharp: just resize, skip the 4-stage pipeline
            enhanced = cv2.resize(raw, TARGET_SIZE, interpolation=cv2.INTER_LANCZOS4)
            print(f"  {fp.name}: SKIPPED (sharpness={raw_sharpness:.1f} > {SHARP_SKIP_THRESHOLD})")
        else:
